...
int mem_group_matesw(const mem_opt_t *opt, int64_t pacLen, const mem_pestat_t pes[4], int groupSize, int** seqLenPairs, uint8_t*** seqsTransPairs, ref_t**** regRefArray, mem_alnreg_v*** alnRegVecPairs) {
  ...
  if((opt->flag & MEM_F_NO_RESCUE) == 0) { // then perform SW for the best alignment
    for(k = 0; k < groupSize; k++) {
      mem_alnreg_v alnRegTmpVecPairs[2];
      kv_init(alnRegTmpVecPairs[0]); 
      kv_init(alnRegTmpVecPairs[1]);

      int i = 0;      
      int j = 0;
      for(i = 0; i < 2; i++) {
        int j = 0;
        for (j = 0; j < (*alnRegVecPairs)[k][i].n; ++j)
          if ((*alnRegVecPairs)[k][i].a[j].score >= (*alnRegVecPairs)[k][i].a[0].score  - opt->pen_unpaired)
            kv_push(mem_alnreg_t, alnRegTmpVecPairs[i], (*alnRegVecPairs)[k][i].a[j]);
      }
 
      for(i = 0; i < 2; i++) 
        for(j = 0; j < alnRegTmpVecPairs[i].n && j < opt->max_matesw; j++) 
          n += mem_matesw_precompute(opt, pacLen, pes, &(alnRegTmpVecPairs[i].a[j]), seqLenPairs[k][!i], (uint8_t*)seqsTransPairs[k][!i], &((*alnRegVecPairs)[k][!i]), regRefArray[k][i][j]);        
      
      free(alnRegTmpVecPairs[0].a); free(alnRegTmpVecPairs[1].a);

// Old version
/*
      for(i = 0; i < 2; i++) {
        for(j = 0; j < (*alnRegTmpVecPairs)[k][i].n && j < opt->max_matesw; j++) {
          //fprintf(stderr, "Reg: %ld %ld %d %d %d %d %d %d %d %d %d %d %"PRIu64"\n", (*alnRegTmpVecPairs)[k][i].a[j].rb, (*alnRegTmpVecPairs)[k][i].a[j].re, (*alnRegTmpVecPairs)[k][i].a[j].qb, (*alnRegTmpVecPairs)[k][i].a[j].qe, (*alnRegTmpVecPairs)[k][i].a[j].score, (*alnRegTmpVecPairs)[k][i].a[j].truesc, (*alnRegTmpVecPairs)[k][i].a[j].sub, (*alnRegTmpVecPairs)[k][i].a[j].csub, (*alnRegTmpVecPairs)[k][i].a[j].sub_n, (*alnRegTmpVecPairs)[k][i].a[j].w, (*alnRegTmpVecPairs)[k][i].a[j].seedcov, (*alnRegTmpVecPairs)[k][i].a[j].secondary, (*alnRegTmpVecPairs)[k][i].a[j].hash);
          n += mem_matesw_precompute(opt, pacLen, pes, &((*alnRegTmpVecPairs)[k][i].a[j]), seqLenPairs[k][!i], (uint8_t*)seqsTransPairs[k][!i], &((*alnRegVecPairs)[k][!i]), regRefArray[k][i][j]);
        }
...
#ifdef DEBUG
      if((k % 100) == 0) fprintf(stderr, "[C] Mate-SW: %d\n", k);
#endif
    }
  }
}
...
*/
    //fprintf(stderr, "r=%d, rb: %ld, re: %ld, len: %ld\n", r, ref[r].rBeg, ref[r].rEnd, ref[r].len);
		if (ref[r].len == ref[r].rEnd - ref[r].rBeg) { // no funny things happening
			kswr_t aln;
			mem_alnreg_t b;
			int tmp, xtra = KSW_XSUBO | KSW_XSTART | (l_ms * opt->a < 250? KSW_XBYTE : 0) | (opt->min_seed_len * opt->a);
			aln = ksw_align2(l_ms, seq, ref[r].len, ref[r].ref, 5, opt->mat, opt->o_del, opt->e_del, opt->o_ins, opt->e_ins, xtra, 0);
			memset(&b, 0, sizeof(mem_alnreg_t));
			if (aln.score >= opt->min_seed_len && aln.qb >= 0) { // something goes wrong if aln.qb < 0
				b.qb = is_rev? l_ms - (aln.qe + 1) : aln.qb;                                                                                                                                                                              
				b.qe = is_rev? l_ms - aln.qb : aln.qe + 1; 
				b.rb = is_rev? (l_pac<<1) - (ref[r].rBeg + aln.te + 1) : ref[r].rBeg + aln.tb;
				b.re = is_rev? (l_pac<<1) - (ref[r].rBeg + aln.tb) : ref[r].rBeg + aln.te + 1;
				b.score = aln.score;
				b.csub = aln.score2;
				b.secondary = -1;
